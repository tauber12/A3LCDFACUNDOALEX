...
void LED_Config(void){
	...
	LED_PORT -> MODER &= ~(GPIO_MODER_MODE8);
	LED_PORT -> MODER |= (GPIO_MODER_MODE8_0);
	LED_PORT -> OTYPER &= ~(GPIO_OTYPER_OT8);
	LED_PORT -> PUPDR &= ~(GPIO_PUPDR_PUPD8);
	LED_PORT -> OSPEEDR |= (3 << GPIO_OSPEEDR_OSPEED8_Pos);
	LED_PORT -> BRR |= (GPIO_PIN_8);
